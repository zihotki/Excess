...
namespace Excess.Compiler.Core
{
    ...
    {
        private readonly Dictionary<InstanceConnector, Action<InstanceConnector, object, object, Scope>> _dataTransform =
            new Dictionary<InstanceConnector, Action<InstanceConnector, object, object, Scope>>();

        private Action<InstanceConnection<TNode>, Scope> _defaultInputTransform;
        private Action<InstanceConnection<TNode>, Scope> _defaultOutputTransform;


        private readonly Dictionary<string, InstanceConnector> _input = new Dictionary<string, InstanceConnector>();
        private readonly Func<string, object, Scope, bool> _match;

        private readonly Dictionary<InstanceConnector, Action<InstanceConnector, InstanceConnection<TNode>, Scope>> _nodeTransform =
            new Dictionary<InstanceConnector, Action<InstanceConnector, InstanceConnection<TNode>, Scope>>();

        private readonly Dictionary<string, InstanceConnector> _output = new Dictionary<string, InstanceConnector>();
        private readonly IInstanceAnalisys<TNode> _owner;

        private IInstanceTransform<TNode> _transform;
        ...
        public IInstanceMatch<TNode> Input(InstanceConnector connector, Action<InstanceConnector, object, object, Scope> dt,
        ...
        public IInstanceMatch<TNode> Output(InstanceConnector connector, Action<InstanceConnector, object, object, Scope> dt,
            Action<InstanceConnector, InstanceConnection<TNode>, Scope> transform)
        {
            var id = connector.Id;
        ...
        {
            return
                Then(new FunctorInstanceTransform<TNode>(_input, _output, _dataTransform, _nodeTransform, _defaultInputTransform, _defaultOutputTransform,
                    handler));
        }
